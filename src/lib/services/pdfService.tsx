...
const InsightsPDF = ({ insights, selectedSections }: InsightsPDFProps) => {
  const isSectionEnabled = (sectionId: string) => {
    return (
      selectedSections.find((section) => section.id === sectionId)?.checked ||
      false
    );
  };
        ...
        <View style={styles.section}>
            ...
            <>
              ...
              <View style={styles.statsContainer}>
                <View>
                  ...
                  <Text style={styles.text}>
                    Win Rate: {Math.round(insights.winLossRatio.white.winRate)}%
                  </Text>
                </View>
                <View>
                  ...
                  <Text style={styles.text}>
                    Win Rate: {Math.round(insights.winLossRatio.black.winRate)}%
                  </Text>
                </View>
              </View>
            </>
          ...
          {isSectionEnabled("accuracy") && (
            <>
              ...
              <Text style={styles.text}>
                Overall: {Math.round(insights.accuracy.overall)}%
              </Text>
              <Text style={styles.text}>
                As White: {Math.round(insights.accuracy.asWhite)}%
              </Text>
              <Text style={styles.text}>
                As Black: {Math.round(insights.accuracy.asBlack)}%
              </Text>
            </>
          )}
            ...
            <>
              ...
              {insights.openings.mostPlayed.map((opening, index) => (
                <Text key={index} style={styles.text}>
                  {opening.name} - Played: {opening.count}, Win Rate:{" "}
          ...
          {isSectionEnabled("weaknesses") && (
            <>
              ...
              {insights.weaknesses.map((weakness, index) => (
                <View key={index}>
                  <Text style={styles.text}>
                    {weakness.phase.charAt(0).toUpperCase() +
                      weakness.phase.slice(1)}{" "}
                    Phase:
                  </Text>
                  <Text style={styles.text}>
                  ...
                  <Text style={styles.text}>
                    Average Eval Drop: {weakness.averageEvalDrop.toFixed(2)}
                  </Text>
                </View>
              ))}
            </>
...
export const generateInsightsPDF = (
  ...
  return (
    <InsightsPDF insights={insights} selectedSections={selectedSections} />
  );
};
