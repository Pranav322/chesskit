...
export class AutoTagService {
  ...
  private parseTimeControl(pgn: string): AutoTagResult['timeControl'] | undefined {
    ...
    const totalTime = baseTime + increment * 40; // Estimate for 40 moves
    
    let type: AutoTagResult['timeControl']['type'];
    if (totalTime <= 180) type = 'bullet';      // 3 minutes or less
    else if (totalTime <= 600) type = 'blitz';  // 10 minutes or less
    else if (totalTime <= 1800) type = 'rapid'; // 30 minutes or less
    else type = 'classical';
  ...
  private hasMaterialImbalance(fen: string): boolean {
    ...
    for (const char of position) {
      if (pieces.white.hasOwnProperty(char)) pieces.white[char as keyof typeof pieces.white]++;
      if (pieces.black.hasOwnProperty(char)) pieces.black[char as keyof typeof pieces.black]++;
    }

    // Check for significant material imbalance
    const whiteMaterial = pieces.white.P + 3 * pieces.white.N + 3 * pieces.white.B + 
                         5 * pieces.white.R + 9 * pieces.white.Q;
    const blackMaterial = pieces.black.p + 3 * pieces.black.n + 3 * pieces.black.b + 
                         5 * pieces.black.r + 9 * pieces.black.q;
